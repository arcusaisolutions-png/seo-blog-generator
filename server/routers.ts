...
const voiceDnaSchema = z.object({
  ...
  opinionated: z.number().min(0).max(100),
  elaborate: z.number().min(0).max(100),
...
const voiceAnalysisSchema = z.object({
  ...
  preferredClosings: z.array(z.string()),
  ...
  toneProfile: z.object({
    formalToCasual: z.number(),
    reservedToBold: z.number(),
    neutralToOpinionated: z.number(),
    dryToPlayful: z.number(),
    softToAuthoritative: z.number(),
    conciseToElaborate: z.number(),
  }),
  styleProfile: z.object({
    avgSentenceLength: z.number(),
    avgParagraphLength: z.number(),
    rhetoricalQuestionFrequency: z.number(),
    storytellingLevel: z.number(),
    metaphorLevel: z.number(),
    readabilityLevel: z.string(),
    vocabularyComplexity: z.string(),
    formattingPreference: z.array(z.string()),
  }),
...
async function resolveVoiceSelection(
  ...
  const secondary = await getVoiceProfileById(
    selection.secondaryVoiceProfileId,
    userId
  );
  if (!secondary) throw new Error("Secondary voice profile not found");
  return {
    voice,
    conditioning: synthesizeVoiceConditioning(
      voice,
      secondary,
      selection.primaryVoiceWeight ?? 50
    ),
  };
}

function buildBlogGenerationInput(
  draft: BlogDraft,
  voice: VoiceProfile | null,
...
export const appRouter = router({
  ...
  workspace: router({
    list: publicProcedure.query(({ ctx }) =>
      getUserWorkspaces(ctx.user?.id || 1)
    ),
    getDefault: publicProcedure.query(({ ctx }) =>
      getOrCreateDefaultWorkspace(ctx.user?.id || 1)
    ),
  }),
  ...
  voice: router({
    list: publicProcedure
      ...
      .query(({ ctx, input }) =>
        getUserVoiceProfiles(ctx.user?.id || 1, input?.search)
      ),

    get: publicProcedure
      .input(z.object({ id: z.number() }))
      .query(({ ctx, input }) =>
        getVoiceProfileById(input.id, ctx.user?.id || 1)
      ),
    ...
    extractFiles: publicProcedure
      ...
      .mutation(async ({ input }) => ({
        files: await extractUploadedTexts(input.files),
      })),
    ...
    create: publicProcedure
      .input(
        z.object({
          ...
          voiceType: z
            .enum(["personal", "brand", "team", "campaign"])
            .default("personal"),
    ...
    saveAnalysis: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const userId = ctx.user?.id || 1;
        const combinedText =
          input.sourceTextCombined ??
          sourceSamples?.map(sample => sample.content).join("\n\n") ??
          "";
    ...
    delete: publicProcedure
      .input(z.object({ id: z.number() }))
      .mutation(({ ctx, input }) =>
        deleteVoiceProfile(input.id, ctx.user?.id || 1)
      ),
  ...
  blog: router({
    list: publicProcedure
      ...
      .query(({ ctx, input }) =>
        getUserBlogDrafts(ctx.user?.id || 1, input?.search, input?.status)
      ),
    ...
    update: publicProcedure
      .input(
        z.object({
          ...
          status: z
            .enum(["brief", "outline", "draft", "final", "published"])
            .optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          data.voiceProfileId !== undefined ||
          data.secondaryVoiceProfileId !== undefined ||
          data.primaryVoiceWeight !== undefined
        ) {
          const selection = await resolveVoiceSelection(userId, {
            voiceProfileId:
              data.voiceProfileId === undefined
                ? existing.voiceProfileId
                : data.voiceProfileId,
            secondaryVoiceProfileId:
              data.secondaryVoiceProfileId === undefined
                ? existing.secondaryVoiceProfileId
                : data.secondaryVoiceProfileId,
            primaryVoiceWeight:
              data.primaryVoiceWeight === undefined
                ? existing.primaryVoiceWeight
                : data.primaryVoiceWeight,
          });
          updateData.voiceConditioning =
            selection.conditioning as unknown as Record<string, unknown> | null;
          updateData.primaryVoiceWeight =
            selection.conditioning?.primaryWeight ?? 100;
          updateData.secondaryVoiceWeight =
            selection.conditioning?.secondaryWeight ?? 0;
        }
    ...
    delete: publicProcedure
      .input(z.object({ id: z.number() }))
      .mutation(({ ctx, input }) =>
        deleteBlogDraft(input.id, ctx.user?.id || 1)
      ),
    ...
    generateBrief: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await updateBlogDraft(input.draftId, userId, {
          contentBrief: brief,
          status: "brief",
          voiceConditioning: selection.conditioning as unknown as Record<
            string,
            unknown
          > | null,
        });
    ...
    generateSeo: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const seo = await generateSeoEnhancement(
          content,
          buildBlogGenerationInput(draft, null, null)
        );
        await updateBlogDraft(input.draftId, userId, {
        ...
        try {
          ...
          const inlinePrompts = draft.inlineImagePromptsEnabled
            ? await Promise.all(
                prompts.sections.map((prompt, index) =>
                  createGeneratedImage({
                    userId,
  ...
  repurpose: router({
    list: publicProcedure.query(({ ctx }) =>
      getUserRepurposeSessions(ctx.user?.id || 1)
    ),

    get: publicProcedure
      .input(z.object({ id: z.number() }))
      .query(({ ctx, input }) =>
        getRepurposeSessionById(input.id, ctx.user?.id || 1)
      ),
    ...
    generatePlan: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await updateRepurposeSession(input.sessionId, userId, {
          status: "planning",
        });
    ...
    generateContent: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await logUsage({
          ...
          resourceType: "repurpose_session",
  ...
  image: router({
    list: publicProcedure.query(({ ctx }) =>
      getUserGeneratedImages(ctx.user?.id || 1)
    ),
    ...
    generate: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const prompt = [
          input.prompt,
          input.variation
            ? "Create a distinct visual variation while preserving the subject and intent."
            : "",
          `Compose the image for a ${aspectRatio} aspect ratio.`,
        ]
          .filter(Boolean)
    ...
    delete: publicProcedure
      .input(z.object({ id: z.number() }))
      .mutation(({ ctx, input }) =>
        deleteGeneratedImage(input.id, ctx.user?.id || (1 as number))
      ),
  }),
  ...
  template: router({
    list: publicProcedure
      ...
      .query(({ ctx, input }) =>
        getUserTemplates(ctx.user?.id || 1, input?.search, input?.type)
      ),
    ...
    delete: publicProcedure
      .input(z.object({ id: z.number() }))
      .mutation(({ ctx, input }) =>
        deleteTemplate(input.id, ctx.user?.id || (1 as number))
      ),
  }),
    ...
    update: publicProcedure
      .input(
        z.object({
          ...
          englishVariant: z
            .enum(["american", "british", "australian", "canadian"])
            .optional(),
      ...
      .mutation(({ ctx, input }) =>
        upsertUserSettings(ctx.user?.id || 1, input as any)
      ),
  }),
  ...
  usage: router({
    stats: publicProcedure.query(({ ctx }) =>
      getUserUsageStats(ctx.user?.id || 1)
    ),
    logs: publicProcedure
      .input(z.object({ limit: z.number().optional() }).optional())
      .query(({ ctx, input }) =>
        getUserUsageLogs(ctx.user?.id || 1, input?.limit)
      ),
    exportHistory: publicProcedure.query(({ ctx }) =>
      getUserExportHistory(ctx.user?.id || 1)
    ),
  }),
  ...
  seed: router({
    ...
    resetAndReseed: publicProcedure.mutation(async ({ ctx }) => {
      ...
      if (db) {
        const { voiceProfiles, blogDrafts, templates } = await import(
          "../drizzle/schema"
        );
        ...
        await db
          ...
          .where(
            and(
              eq(voiceProfiles.userId, ctx.user?.id || 1),
              eq(voiceProfiles.isDemo, true)
            )
          );
        await db
          .delete(blogDrafts)
          .where(
            and(
              eq(blogDrafts.userId, ctx.user?.id || 1),
              eq(blogDrafts.isDemo, true)
            )
          );
        await db
          .delete(templates)
          .where(
            and(
              eq(templates.userId, ctx.user?.id || 1),
