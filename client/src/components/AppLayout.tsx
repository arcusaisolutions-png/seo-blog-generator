...
export default function AppLayout({ user, children }: AppLayoutProps) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      {/* Logo */}
      <div
        className={`flex items-center gap-2.5 px-4 py-4 ${collapsed ? "justify-center" : ""}`}
      >
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-fuchsia-500 to-cyan-400 flex items-center justify-center flex-shrink-0 shadow-lg shadow-fuchsia-500/20">
                ...
                <Link href={path}>
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                      isActive
                        ? "bg-fuchsia-500/15 text-fuchsia-200 ring-1 ring-fuchsia-400/35"
      ...
      <div className="p-2 border-t border-border">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-md hover:bg-accent transition-colors ${collapsed ? "justify-center" : ""}`}
            >
  ...
  return (
    <div className="studio-app flex h-screen bg-[#050510] overflow-hidden">
      {/* Desktop Sidebar */}
      <aside
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-14 border-b border-border bg-card/50 backdrop-blur-sm flex items-center px-4 gap-3 flex-shrink-0">
          ...
          <Link href="/new-blog">
            <Button
              size="sm"
              className="gap-1.5 bg-gradient-to-r from-fuchsia-500 to-cyan-400 text-white hover:from-fuchsia-400 hover:to-cyan-300"
            >
